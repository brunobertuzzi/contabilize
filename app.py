...
from scripts.database import LoginAttempt, User, get_db
...
def create_app(testing=False):
    ...
    from scripts.empresa_service import empresa_bp
    ...
    def add_security_headers(response):
        ...
        # Evitar cache para garantir que logout/login funcione corretamente e testes não peguem versões antigas
    ...
    def not_found_error(error):
        ...
        return jsonify(
            {
                "error": "Not Found",
                "message": "A URL solicitada não foi encontrada no servidor.",
            }
        ), 404
    ...
    def internal_error(error):
        ...
        return jsonify(
            {"error": "Internal Server Error", "message": "Ocorreu um erro inesperado."}
        ), 500
        ...
        try:
            with get_db() as db:
                return LoginAttempt.is_locked_out(
                    ...
                    window_minutes=int(
                        Config.LOGIN_LOCKOUT_DURATION.total_seconds() / 60
                    ),
                )
        except Exception as e:
    ...
    def setup_admin():
        ...
        if request.method == "POST":
            ...
            try:
                ...
                flash(
                    "Usuário administrador criado com sucesso! Faça login para continuar.",
                    "success",
                )
                return redirect(url_for("login"))
            except ValueError as e:
    ...
    def login():
        ...
        if request.method == "POST":
            ...
            if is_locked:
                flash(
                    f"Conta temporariamente bloqueada. Tente novamente em {remaining_minutes} minutos.",
                    "danger",
                )
                return render_template("login.html")
            ...
            try:
                with get_db() as db:
                    ...
                    if user and user.check_password(password):
                        ...
                        logging.info(
                            f"AUDIT: Login bem-sucedido para usuário: {user.username} de IP: {request.remote_addr}"
                        )
                        ...
                        if user:
                            logging.warning(
                                f"AUDIT: Senha incorreta para o usuário: {username} de IP: {request.remote_addr}"
                            )
                        else:
                            logging.warning(
                                f"AUDIT: Tentativa de login com usuário inexistente: {username} de IP: {request.remote_addr}"
                            )

                        if remaining > 0:
                            flash(
                                f"Nome de usuário ou senha inválidos. Tentativas restantes: {remaining}",
                                "danger",
                            )
                        else:
                            flash(
...
def start_application():
    ...
    parser = argparse.ArgumentParser(description="Contabilize Application")
    parser.add_argument(
        "--dev",
        action="store_true",
        help="Modo desenvolvimento (Flask direto na porta 5000)",
    )
    parser.add_argument(
        "--port", type=int, default=5000, help="Porta para modo dev (padrão: 5000)"
    )
    ...
    try:
        ...
        if args.dev:
            # Modo desenvolvimento - Flask direto (para testes)
            logging.info(f"Iniciando em modo desenvolvimento na porta {args.port}")
            app.run(host="127.0.0.1", port=args.port, debug=False, threaded=True)
        else:
            # Modo produção - PyWebView GUI
            start_gui(app)
