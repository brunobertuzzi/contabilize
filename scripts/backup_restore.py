...
def backup_database():
    ...
    try:
        ...
        return jsonify({'success': False, 'message': 'Erro interno ao criar backup.'}), 500
...
def restore_database():
    ...
    try:
        ...
        if not backup_file.filename.endswith('.db'):
            return jsonify({'success': False, 'message': 'Formato inválido. Apenas arquivos .db são aceitos.'}), 400
        ...
        try:
            ...
            if os.path.getsize(temp_path) > MAX_BACKUP_SIZE:
                return jsonify({'success': False, 'message': 'Arquivo muito grande. Máximo: 100MB.'}), 400
            
            # Valida integridade SQLite
            if not _validate_sqlite_file(temp_path):
                current_app.logger.warning(f"SECURITY: Tentativa de restauração com arquivo inválido: {backup_file.filename}")
                return jsonify({'success': False, 'message': 'Arquivo de backup inválido ou corrompido.'}), 400
            ...
            if os.path.exists(db_path):
                ...
                current_app.logger.info(f"AUDIT: Backup de segurança criado antes da restauração: {safety_backup}")
            
            # Substitui o banco de dados
            shutil.copy(temp_path, db_path)
            
            current_app.logger.info(f"AUDIT: Banco de dados restaurado a partir de: {backup_file.filename}")
        ...
        return jsonify({'success': False, 'message': 'Erro interno ao restaurar banco de dados.'}), 500
