...
def has_admin_user():
    ...
    with get_db() as db:
        return db.query(User).filter_by(is_admin=True).first() is not None
...
def validate_password_strength(password):
    ...
    if strength_count < 2:
        raise ValueError("Senha deve conter pelo menos 2 tipos de caracteres: maiúsculas, minúsculas, números ou símbolos")
